...
type ReplyDrawerState = {
  mode: "add" | "edit" | "detail" | "";
  id?: string;
  open: boolean;
}
type ReplyDrawerProps = ReplyDrawerState & {
  onClose: () => void
  onSubmit: () => void
}

function ReplyDrawer(props: ReplyDrawerProps) {
  ...
  const [replyDetail, setReplyDetail] = useState({} as ResReplyDetail);

  const getDetail = () => {
    setDrawerLoading(true)
    findReply({id}).then(res => {
      const {data} = res;
  ...
  const onAddReply = (params: ReqAddReplyBody) => {
    return addReply(params).then(res => {
      message.success("添加成功")
      return res
    })
  }

  const onUpdateReply = (params: ReqUpdateReplyBody) => {
    setDrawerLoading(true)
    return updateReply(params).then(res => {
  ...
  const submitForm = (formData: ReqAddReplyBody) => {
    setDrawerLoading(true)
    Promise.resolve().then(() => {
      return mode == "add"
        ? onAddReply(formData)
        : onUpdateReply({id, ...formData})
    }).then(_ => {
      onSubmit()
      handleClose()
    }).finally(() => setDrawerLoading(false))
  ...
  const handleClose = () => {
    setReplyDetail({} as ResReplyDetail)
  ...
  const genDetail = () => (
    <>
      <Descriptions bordered>
        <Descriptions.Item label="id" span={4}>{replyDetail.id}</Descriptions.Item>
        <Descriptions.Item label="权限名" span={2}>{replyDetail.name}</Descriptions.Item>
        <Descriptions.Item label="类型" span={2}>{replyDetail.type}</Descriptions.Item>
        <Descriptions.Item label="创建时间" span={2}>{replyDetail.createdAt}</Descriptions.Item>
        <Descriptions.Item label="修改时间" span={2}>{replyDetail.updatedAt}</Descriptions.Item>
        <Descriptions.Item label="描述" span={4}>{replyDetail.description}</Descriptions.Item>
      </Descriptions>
    </>
  )
...
function genColumns(onDetail: any, onEdit: any, onDelete: any): ColumnsType<ResReplyPaging> {
  return [
    {
      title: '权限名',
      dataIndex: 'name',
...
function ReplyManager() {
  ...
  const [drawerState, setDrawerState] = useState({id: "", mode: "detail", open: false} as ReplyDrawerState);
  const [isLoading, setIsLoading] = useState(false);
  const [selTableKeys, setSelTableKeys] = useState([] as any[]);
  const ref = useRef() as any
  const [model, modelContextHolder] = Modal.useModal()

  const onSearch = (params: ReqFindReplyPagingBody) => {
    return findReplyPaging(params).then(res => res.data) as Promise<ResTableData<ResUserBaseInfo>>
  }
  ...
  return (
    <SearchTableTemp
      ...
      <ReplyDrawer onSubmit={onDataChange} onClose={() => {
        setDrawerState({
          mode: "", open: false, id: ""
        })
      }} {...drawerState}/>
