...
type CommentDrawerProps = CommentDrawerState & {
  onClose: () => void
  onSubmit: () => void
}
...
function CommentDrawer(props: CommentDrawerProps) {
  ...
  const [commentDetail, setCommentDetail] = useState({} as ResCommentDetail);

  const getDetail = () => {
    setDrawerLoading(true)
    findComment({id}).then(res => {
      const {data} = res;
  ...
  const onAddComment = (params: ReqAddCommentBody) => {
  ...
  const onUpdateComment = (params: ReqUpdateCommentBody) => {
    setDrawerLoading(true)
    return updateComment(params).then(res => {
  ...
  const submitForm = (formData: ReqAddCommentBody) => {
    setDrawerLoading(true)
    Promise.resolve().then(() => {
      return mode == "add"
        ? onAddComment(formData)
        : onUpdateComment({id, ...formData})
    }).then(_ => {
      onSubmit()
      handleClose()
    }).finally(() => setDrawerLoading(false))
  ...
  const handleClose = () => {
    setCommentDetail({} as ResCommentDetail)
  ...
  const genDetail = () => (
    <>
      <Descriptions bordered>
        <Descriptions.Item label="id" span={4}>{commentDetail.id}</Descriptions.Item>
        <Descriptions.Item label="权限名" span={2}>{commentDetail.name}</Descriptions.Item>
        <Descriptions.Item label="类型" span={2}>{commentDetail.type}</Descriptions.Item>
        <Descriptions.Item label="创建时间" span={2}>{commentDetail.createdAt}</Descriptions.Item>
        <Descriptions.Item label="修改时间" span={2}>{commentDetail.updatedAt}</Descriptions.Item>
        <Descriptions.Item label="描述" span={4}>{commentDetail.description}</Descriptions.Item>
      </Descriptions>
    </>
  )
...
function genColumns(onDetail: any, onEdit: any, onDelete: any): ColumnsType<ResCommentPaging> {
  return [
    {
      title: '权限名',
      dataIndex: 'name',
...
function CommentManager() {
  ...
  const [drawerState, setDrawerState] = useState({id: "", mode: "detail", open: false} as CommentDrawerState);
  const [isLoading, setIsLoading] = useState(false);
  const [selTableKeys, setSelTableKeys] = useState([] as any[]);
  const ref = useRef() as any
  const [model, modelContextHolder] = Modal.useModal()

  const onSearch = (params: ReqFindCommentPagingBody) => {
    return findCommentPaging(params).then(res => res.data) as Promise<ResTableData<ResUserBaseInfo>>
  }
  ...
  return (
    <SearchTableTemp
      ...
      <CommentDrawer onSubmit={onDataChange} onClose={() => {
        setDrawerState({
          mode: "", open: false, id: ""
        })
      }} {...drawerState}/>
